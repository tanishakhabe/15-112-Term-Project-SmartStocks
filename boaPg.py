...
def boaDisplay_redrawAll(app):
    #Display the stock name and symbol. 
    drawLabel(f'Bank of America Corporation ({boa.symbol}): ', app.cx, 50, size=25, font='monospace')
    drawImage(app.boaImage, app.cx, 450, width = 200, height = 75, align='center')

    #Display the cut up description so it doesn't run off screen.
    startX = 150
    space = 20
    cutDesc = boa.description.split(' ')
    ...
    drawRect(app.cx, 115, app.loginW, app.loginH, align='center', fill= 'thistle' if app.boaFaved else None, border='black',)
    drawLabel('Favorite this stock!', app.cx, 115, size=16, bold=True, font='monospace')

def boaDisplay_onMousePress(app, mouseX, mouseY):
    if (app.cx - app.loginW/ 2) <= mouseX <= (app.cx + app.loginW / 2) and (115 - app.loginH / 2) <= mouseY <= (115 + app.loginH / 2):
        app.boaFaved = not app.boaFaved
        if app.boaFaved: 
            app.currUser.favorites.append(boa.name)
        else:
            app.currUser.favorites.remove(boa.name)

def boaDisplay_onKeyPress(app, key):
    if key == 'g':
        app.boaGraph = True
        if app.boaGraph:
            setActiveScreen('boaGraph')
    if key == 'b':
        setActiveScreen('recsPg')
    if key == 'f':
        setActiveScreen('favsPg')

def boaGraph_redrawAll(app):
    drawLabel(f'Bank of America Corporation ({boa.symbol}) Stock Price History', app.cx, 50, size=20, font='monospace')
    ...
    #Drawing y-axis labels for normal size graph
        for i in range(len(app.yAxisLabelsBOA)):
            drawLabel(f'{app.yAxisLabelsBOA[i]}', 80, app.normYAxisBOA[i], align='center', size=16)

    #Drawing the normal points on the graph. 
        newBOAPts = scaling(pricesDict['boa'], app.normYAxisBOA[1], app.normYAxisBOA[2]) 
        for i in range (1, len(newBOAPts)):
    ...
    if app.zoomed: 
        ...
        for i in range(len(app.yAxisLabelsBOA)):
            drawLabel(f'{app.yAxisLabelsBOA[i]}', 40, app.zoomedYAxisBOA[i], align='center', size=12)

        #Drawing the zoomed points on the graph. 
        newZoomedBOAPts = scaling(pricesDict['boa'], app.zoomedYAxisBOA[1], app.zoomedYAxisBOA[2])
        for i in range (1, len(newZoomedBOAPts)):
            x1 = app.zoomedXAxis[i-1]
            y1 = newZoomedBOAPts[i-1]
            x2 = app.zoomedXAxis[i]
            y2 = newZoomedBOAPts[i]
            drawLine(x1, y1, x2, y2)

        # Check if a point has been clicked in normal mode for BOA to draw the pop up box. 
    if app.zoomedBoxBOA == True and app.clickedLineSegBOA != None: 
        # Draw the small pop-up box with the surrounding area zoomed in. 
        (oldX1, oldY1), (oldX2, oldY2) = app.clickedLineSegBOA
        ...
        for i in range(1, len(scaledXs)):
            ...
            drawLabel(f"${pricesDict['boa'][app.selectedIndexBOA]}",(oldX1 + oldX2) / 2 + 80,(oldY1 + oldY2) / 2 - 80, align='center', size=12 )

def boaGraph_onMousePress(app, mouseX, mouseY):
    for i in range(1, len(app.normXAxis)):
        x1, y1 = app.normXAxis[i - 1], app.newBOAPts[i - 1]
        x2, y2 = app.normXAxis[i], app.newBOAPts[i]
        if x1 <= mouseX <= x2 and min(y1, y2) <= mouseY <= max(y1, y2):
            break
    app.selectedIndexBOA = i 
    app.clickedLineSegBOA = ((x1, y1), (x2, y2))
    app.zoomedBoxBOA = not app.zoomedBoxBOA

def boaGraph_onKeyPress(app, key):
    if key == 'b':
